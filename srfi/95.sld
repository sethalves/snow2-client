(define-library (srfi 95)
  ...
  (begin
    ...
    (define (array-dimensions obj)
      (cond ((string? obj) (list (string-length obj)))
            ((vector? obj) (list (vector-length obj)))
            ((bytevector? obj) (list (bytevector-length obj)))
            (else
             (error "array-dimensions -- not an array." obj))))


    (define (array-ref obj idx)
      (cond ((string? obj) (string-ref obj idx))
            ((vector? obj) (vector-ref obj idx))
            ((bytevector? obj) (bytevector-u8-ref obj idx))
            (else
             (error "array-ref -- not an array." obj))))


    (define (array-set! obj v idx)
      (cond ((string? obj) (string-set! obj idx v))
            ((vector? obj) (vector-set! obj idx v))
            ((bytevector? obj) (bytevector-u8-set! obj idx v))
            (else
             (error "array-set! -- not an array." obj))))


    (define (make-array obj . dims)
      (cond ((string? obj) (make-string (car dims)))
            ((vector? obj) (make-vector (car dims)))
            ((bytevector? obj) (make-bytevector (car dims)))
            (else
             (error "make-array -- not an array." obj))))
    ...
    (define (sort seq less? . opt-key)
      ...
      (cond ((array? seq)
             (let* ((dims (array-dimensions seq)))
               (define newra (apply make-array (cons seq dims)))
               (do ((sorted (sort:sort-list! (rank-1-array->list seq) less? key)
                            (cdr sorted))
                    (i 0 (+ i 1)))
                   ((null? sorted) newra)
