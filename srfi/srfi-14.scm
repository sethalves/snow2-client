...
;;;   (lambda (pred val caller) (if (not (pred val)) (error val caller) val))
...
(define (check-arg pred val caller)
  (if (not (pred val)) (error val caller) val))
...
(define (char-set-hash cs . maybe-bound)
  ...
  ;;                                                               (exact? n)
  ;;                                                               (<= 0 n)))))
  (let* ((bound (if (pair? maybe-bound) (car maybe-bound) 0))
...
(define (ucs-range->char-set lower upper . rest)
  ...
  (let-values (((error? rest) (if (pair? rest)
