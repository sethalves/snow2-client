;; 37.scm -- Slightly extended variant of SRFI 37
;;
;; Copyright (C) 2014 Jesse Towner <townerj at gmail.com>
;; Copyright (C) 2009, 2010 Andreas Rottmann <a.rottmann at gmx.at>
...
;;   argument list.
;;
;; - Modified to remove dependency on SRFI 1, by using the `member'
;;   procedure for list searching, instead of 'find'.
...
(define (args-fold* args
  ...
  (define (find-option name)
    (let ((result (member name options (lambda (test-name option)
                                         (member test-name (option-names option))))))
      (if (and (list? result)
               (not (null? result)))
          (car result)
          #f)))
