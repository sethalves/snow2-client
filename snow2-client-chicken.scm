...
(define (library-from-name repository library-name)
  (let ((package (find-package-with-library repository library-name)))
    (cond ((not package)
           (format #t "can't find package that contains ~S\n" library-name)
           #f)
          (else
           (let loop ((libraries (snow2-package-libraries package)))
             (cond ((null? libraries) #f)
  ...
  (let ((lib-name-ht (make-hash-table))
    ...
    (let* ((result-names (hash-table-keys lib-name-ht))
      ...
      (cond ((= (length result) (length libraries))
             (format #t "libs: ~S\n packages: ~S\n"
                     (hash-table->alist lib-name-ht)
                     (hash-table->alist package-url-ht))
...
(define (snow2-install repository library-name)
  (format #t "(snow2-install ~S ~S)\n" repository library-name)
  (let ((package (find-package-with-library repository library-name)))
    (cond ((not package)
           (format #t "didn't find a package with library: ~S\n"
                   library-name))
          (else
...
(define (main-program)
  (let* ((repository-url
         ...
         (args (program-and-command-line))
         (operation (list-ref args 1))
         (library-name (read-from-string (list-ref args 2))))
