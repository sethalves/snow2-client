(define-library (seth crypt sha-2)
  (export sha-224
          sha-256)
  ...
  (begin
     ...
     (chicken
      (define (sha-256 src)
        (let ((in (cond ((string? src)
          ...
          (message-digest-port (sha256-primitive)  in 'u8vector))
        ))
     (else))
    ...
    (cond-expand
     (gauche
      ...
      (define (sha-2-worker src algo)
        (string->latin-1
         (cond ((string? src)
                (digest-string algo src))
               (else
                (let ((in (cond ((bytevector? src)
                  ...
                  (let ((result (digest algo)))
                    (current-input-port save-cip)
                    result))))))

      (define (sha-224 src) (sha-2-worker src <sha224>))
    ...
    (cond-expand
     ...
     (sagittarius
      ...
      (define (sha-worker src algo output-size)
        (let ((in (cond ((string? src)
                         (open-input-bytevector (string->utf8 src)))
                        ((bytevector? src)
                         (open-input-bytevector src))
                        ((input-port? src) src)
                        (else (error "unknown digest source: " src))))
              (out (make-bytevector output-size 0)))
          (let ((sha-2 algo))
            (hash-init! sha-2)
            (let loop ()
              (let ((bv (read-bytevector 1024 in)))
                (cond ((eof-object? bv)
                       ...
                       out)
                      (else
                       (hash-process! sha-2 bv)
                       (loop))))))))

      (define (sha-224 src)
        (sha-worker src (hash-algorithm SHA-224) 28))

      (define (sha-256 src)
        (sha-worker src (hash-algorithm SHA-256) 32))
    ...
    ;; bytevector-u32-ref-be is from (chibi bytevector)
              ...
              (cond
               ((< n 64)
                (if (>= n 56)
                    ...
                    ;; return a bytevector instead of a string
                    (let ((bv (make-bytevector (if full? 32 28) 0)))
                      (let loop ((vals (if full?
                                           (list a b c d e f g h)
                                           (list a b c d e f g)))
                                 (i 0))
                        (if (null? vals) bv
                            (do ((x 0 (+ x 1)))
                                ((= x 4) (loop (cdr vals) (+ i 4)))
                              (bytevector-u8-set!
                               bv (+ i x) (byte-extract (car vals) x))))))))
               (else
